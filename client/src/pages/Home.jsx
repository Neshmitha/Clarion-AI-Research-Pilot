...
import { LayoutDashboard, Search, FileText, Upload, Settings, LogOut, Menu, BookOpen, Bot, Edit3, Compass, Star, GitPullRequest } from 'lucide-react';
        ...
        <div className={`flex h-screen font-sans overflow-hidden ${isDark ? 'text-white' : 'text-black'}`} style={{ background: isDark ? '#000000' : '#ffffff' }}>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 ${isDark ? 'bg-black/80 border-white/5' : 'bg-white/80 border-black/5'} backdrop-blur-xl border-r flex flex-col transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 ${isSidebarOpen ? 'lg:flex' : 'lg:hidden'}`}>
                ...
                <nav className="flex-1 p-4 space-y-2">
                    <SidebarItem icon={<LayoutDashboard size={18} />} text="Home" active isDark={isDark} />
